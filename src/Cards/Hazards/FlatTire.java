...
public class FlatTire implements Card {
    ...
    public boolean execute(ArrayList<Player> players, int currentPlayer) {
        ...
        if (!target.checkBattle("None")) {
            ...
            // a Safety card
            if (!target.checkSafeties("Puncture Proof")) {
                target.setBattle("Flat Tire");
            } else {
                System.out.println("Player is protected by Puncture Proof."
                        + " Discarding the card!");
            }
            return true;
        } else {
            System.out.println("Can't play Flat Tire. Try different cards");
            return false;
        }
    }
