...
public class OutofGas implements Card {
    ...
    public boolean execute(ArrayList<Player> players, int currentPlayer) {
        ...
        if (!target.checkBattle("None")) {
            ...
            // a Safety card
            if (!target.checkSafeties("Fuel Tank")) {
                target.setBattle("Out of Gas");
            } else {
                System.out.println("Player is protected by Extra Tank."
                        + " Discarding the card!");
            }
            return true;
        } else {
            System.out.println("Can't play Out of Gas. Try different cards");
            return false;
        }
    }
