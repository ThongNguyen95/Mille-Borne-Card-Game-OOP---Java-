...
public class Accident implements Card {
    ...
    public boolean execute(ArrayList<Player> players, int currentPlayer) {
        ...
        if (!target.checkBattle("None")) {
            ...
            // a Safety card
            if (!target.checkSafeties("Driving Ace")) {
                target.setBattle("Accident");
            } else {
                System.out.println("Player is protected by Driving Ace."
                        + " Discarding the card!");
            }
            return true;
        } else {
            System.out.println("Can't play Accident. Try different cards");
            return false;
        }
    }
