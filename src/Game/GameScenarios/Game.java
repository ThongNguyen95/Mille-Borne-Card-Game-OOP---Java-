...
import Cards.Remedies.EndSpdLimit;
 ...
 * @author Thong Nguyen
 */
//Thiss is the game class with randomized behavior. Scenario is not hard-coded.
public class Game implements BaseGame {

    private boolean gameOver;   //Keep track of game state
    private ArrayDeque<Card> deck; //stack of cards, representing the deck
    ...
    private final List<Player> winners; // In case of tie, display all the winners
    //Constructor
    ...
    public void init() {
        ...
        // This is the setup code for Homework 2. I use another one for HW1
        // Add Distance cards
        for (int i = 0; i < 10; i++) {
            tempDeck.add(new Distance(25));
        }

        for (int i = 0; i < 10; i++) {
            tempDeck.add(new Distance(50));
        }

        for (int i = 0; i < 10; i++) {
            tempDeck.add(new Distance(75));
        }

        for (int i = 0; i < 12; i++) {
            tempDeck.add(new Distance(100));
        }

        for (int i = 0; i < 4; i++) {
            tempDeck.add(new Distance(200));
        }

        // Add Hazard cards
        for (int i = 0; i < 3; i++) {
            tempDeck.add(new Accident());
        }

        for (int i = 0; i < 3; i++) {
            tempDeck.add(new OutofGas());
        }

        for (int i = 0; i < 3; i++) {
            tempDeck.add(new FlatTire());
        }

        for (int i = 0; i < 5; i++) {
            tempDeck.add(new Stop());
        }

        for (int i = 0; i < 4; i++) {
            tempDeck.add(new SpeedLimit());
        }

        // Add Remedies Cards
        for (int i = 0; i < 6; i++) {
            tempDeck.add(new Repairs());
        }

        for (int i = 0; i < 6; i++) {
            tempDeck.add(new Gasoline());
        }

        for (int i = 0; i < 6; i++) {
            tempDeck.add(new SpareTire());
        }

        for (int i = 0; i < 14; i++) {
            tempDeck.add(new Roll());
        }

        for (int i = 0; i < 6; i++) {
            tempDeck.add(new EndSpdLimit());
        }

        // Add Safeties
        tempDeck.add(new DrivingAce());
        tempDeck.add(new FuelTank());
        tempDeck.add(new PunctureProof());
        tempDeck.add(new RightofWay());

        //Add code to the shuffle the deck
    ...
    public String toString() {
        return "Scenario: Randomized Outcome";
    }
}
